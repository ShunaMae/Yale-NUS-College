...
#### (1)Import data
```{r}
eu_nodes <- read_table("email-Eu-core-department-labels.txt.gz", col_names = FALSE)
names(eu_nodes) <- c("id", "depts")
eu_edges <- read_table("email-Eu-core.txt.gz", col_names = FALSE)
names(eu_edges) <- c("from", "to")
```
...
```
ANS: \
The graph is not really informative. There are many overlaps of vertices in the graph, and given that both points and lines are all of the same color, red and black respectively, there is a limited number of distinct lines. Additionally, the points do not have any labels and it is not conveyed which point refers to which `id`. In other words, it is difficult and nearly impossible to understand any relationships from the graph above. Changing the layout makes does not improve the graph, this is because it does not resolve the problem of (1) lack of information for each point, (2) overlap of lines and (3) lines remain indistinguishable. Changing the layout doesn't help much neither, as the plot looks exactly the same even we change the figure dimension. 
...
vertex_attr(contracted_eu_igraph, "depts") <-
...
```
\
The graph is not very informative and is not that useful. While the points are now labelled, the lines connecting the points still overlap one another, rendering them indistinguishable from one another.
