...
p2 <-  ggplot() +
  geom_point(data = male, 
             aes(x=NMDS1, y=NMDS2, colour = name, shape = side_dis), 
             size = 3,
             position = "jitter")+
  labs(title = "Male")+
...
anosim_all = anosim(all_matrix, test$id, distance = "bray", permutations = 9999)
anosim_all
```
...
hclust_f <- hclustf %>%
  filter(sex == 1)%>%
  select(-id, -name, -ID, -hornbill, -sex)
